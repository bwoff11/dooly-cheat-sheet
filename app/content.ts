...
export type CoreGuide = {
  ...
  boardLabels?: { left: string; right: string };
  boardCaption?: string;
  builds: BuildLine[];
  buy: string[];
  hold: string[];
...
export type GlobalItemTier = ItemTier | "C";
...
export type ItemSpotlight = {
  ...
  rule: string;
  threshold: string;
  sequence: { label: string; text: string }[];
  partners: { name: string; why: string }[];
  upgrades: { name: string; why: string }[];
  buyWhen: string[];
  avoidWhen: string[];
...
export const sources = {
  ...
  plasmaRifle: {
    label: "Plasma Rifle — Patch 16.1 live mechanics",
    href: "https://bazaardb.gg/card/szz2k0p3hpz490ffbpyc6s5fwt/Plasma-Rifle",
    type: "Card mechanics",
  },
  trollosaur: {
    label: "Trollosaur — Patch 16.1 live mechanics",
    href: "https://bazaardb.gg/card/ctn94sibhw28l1qmf2j48nxw8/Trollosaur",
    type: "Card mechanics",
  },
  ignitionBuild: {
    label: "Ignition Core build and positioning reference",
    href: "https://mobalytics.gg/the-bazaar/builds/ignition-core-dooley-kripp",
    type: "Expert build",
  },
} as const;
...
export const cores: CoreGuide[] = [
  ...
  },
  {
    ...
    summary:
      "The clearest tempo Core: frequent early Burn wins fights, Plasma Rifle converts application frequency into direct damage, and a supported Trollosaur converts both flat Burn and Damage scaling into a hybrid threat. Speed still matters—an eight-second Silver Core cannot be your opener.",
    plan:
      "Win early with cheap Burn and Blast Doors, bank gold, then spend Gold access on a proven conversion: Plasma Rifle when the first cycle already produces repeated Burn events, or Trollosaur when Damage scaling, acceleration, and a credible destroy trigger are present. Otherwise upgrade the Burn carry or prepare a control or one-shot exit.",
    pickWhen: ["Soldering Gun, Rocket Launcher, Flint Stones, or Blast Doors is already doing work.", "You have Haste or fast left-side triggers.", "Several Burn actions already occur before Plasma Rifle's actual first activation, creating a natural conversion.", "Trollosaur appears beside real Damage scaling, acceleration, or an early destroy enabler such as Diana-Saur.", "You can convert early wins into a large Day 6 shopping budget."],
    avoidWhen: ["Your Burn items are all slow and you have no Haste.", "Plasma Rifle would be the first real Burn payoff rather than a conversion for an existing engine.", "Trollosaur would remain an unsupported nine-second carry with no reliable destruction target.", "You are already losing to sustain and lack a faster late payoff."],
    board: [
      { label: "FAST BURN", kind: "trigger", size: 1 },
      { label: "HASTE / FIBER", kind: "trigger", size: 1 },
      { label: "IGNITION", kind: "core", size: 2 },
      { label: "BURN ENGINE", kind: "trigger", size: 2 },
      { label: "DEF / FLEX", kind: "flex", size: 1 },
      { label: "RIFLE / TROLLO", kind: "payoff", size: 2 },
    ],
    boardLabels: { left: "LEFT · CHARGES IGNITION", right: "RIGHT · BURN SCALES; RIFLE CASHES OUT" },
    boardCaption: "Choose the supported conversion rather than defaulting to both slow Medium carries. Put Trollosaur right of Ignition; for Rifle, arrange equal-time items so Burn resolves first. Rifle is Weapon / Tech—not Burn—and receives no right-side Burn gain.",
    builds: [
      { name: "Grenade / Rocket tempo", label: "Reliable opener", confidence: "consensus", pitch: "Front-loaded and Multicast Burn turns every Core cycle into a larger stack and a strong early lead.", enter: "Plasma Grenade, Rocket Launcher, or Soldering Gun arrives with a fast trigger or Haste item.", engine: "Left triggers cycle Ignition; Core scales the right-side Burn engine before Grenade and Rocket land their opening packet.", mustHave: ["Plasma Grenade, Rocket, or Soldering", "Haste", "fast Core activators"], flex: ["Blast Doors", "Flint Stones", "Levitation Pad", "control"], pivot: "Use this to reach Gold access. If Burn frequency is high but stack damage is no longer enough, test Plasma Rifle instead of adding another small burner." },
      { name: "Plasma Rifle conversion", label: "Supported carry", confidence: "situational", pitch: "Turn every Burn application into fight-long direct-damage scaling without discarding the early engine.", enter: "Plasma Rifle appears and the current board can apply at least three distinct Burn events before its actual first activation after speed effects—six seconds at base speed.", engine: "Ignition and fast Burn sources scale the Rifle before it fires; Metronome, Fiber Optics, Crit, or Haste helps make a second shot possible and threatening.", mustHave: ["Plasma Rifle", "3+ pre-shot Burn events", "Burn-before-Rifle ordering"], flex: ["Metronome", "Fiber Optics", "Crit", "Blast Doors or control"], pivot: "Treat Rifle as a supported midgame conversion, not a blind late force. Replace it when a complete Hammer, Ice 9000, Terry-Dactyl, or higher-cap Launch Pad finish overtakes it." },
      { name: "Trollosaur destroy hybrid", label: "Conditional bridge", confidence: "situational", pitch: "Combine Ignition's flat Burn growth with a weapon whose current Damage also sets its proportional Burn.", enter: "Trollosaur appears with real Damage scaling, Haste or Charge, and either Diana-Saur-style destruction or an opposing Small item likely to survive until the half-Health trigger.", engine: "Place Trollosaur right of Ignition. Core uses add flat Burn, Damage buffs raise both its hit and its 10%-of-Damage Burn, and each successful destruction immediately uses Trollosaur again.", mustHave: ["Trollosaur", "Damage scaling or reliable destroy", "Haste / Charge or eligible Small target"], flex: ["Diana-Saur / Grav Well", "Shiny / Obsidian", "Crit", "defense"], pivot: "Pass or sell if it stays a nine-second 50-Damage / 5-Burn card, the opponent has no eligible Small target, or the fight ends before its first meaningful use." },
      { name: "Burn into control / burst", label: "Exit line", confidence: "situational", pitch: "Use Burn tempo and economy to fund Chronobarrier, Flamethrower, or a faster one-shot when the lobby invalidates damage-over-time.", enter: "Shield, healing, or immediate-kill boards survive both the Burn stack and a supported Rifle's first two shots.", engine: "Early wins preserve health and gold; the late payoff replaces the damage plan while useful Haste, Crit, and control remain.", mustHave: ["premium control or burst payoff", "enough scaling", "clean transition timing"], flex: ["Flamethrower", "Haste", "Crit", "one Burn utility item"], pivot: "Stay Burn if the alternative is under-scaled; a higher-rarity item is not an upgrade unless it improves expected damage or activation timing." },
    ],
    buy: ["Plasma Rifle when 3+ pre-shot Burn events are already live", "Trollosaur only with Damage scaling, acceleration, or reliable destruction", "Plasma Grenade / Rocket Launcher", "Blast Doors", "Metronome / Fiber Optics"],
    hold: ["Soldering Gun while Multicast creates useful Burn events", "Diana-Saur or another credible early destroy trigger for Trollosaur", "Crit support for a scaled Plasma Rifle", "one defensive or control answer"],
    skip: ["unsupported Plasma Rifle with no pre-cast Burn frequency", "naked nine-second Trollosaur with no scaling or eligible Small target", "extra burners after Rifle needs speed, Crit, or survival", "slow Burn with no acceleration"],
    merchants: [
      { name: "Hef", why: "The focused Burn pool contains Trollosaur as a true Burn item, Plasma Rifle through its Burn-reference text, and the frequent engines that scale either conversion.", reroll: "At Gold access, reroll only when Trollosaur or Rifle plus multiple Burn upgrades are acceptable hits.", leave: "Leave when the carry is solved and the missing piece is Damage, speed, Crit, or survival." },
      { name: "Aila", why: "Weapon-only stock can find or upgrade Trollosaur, Plasma Rifle, Rocket Launcher, and other direct-damage conversions.", reroll: "Reroll while multiple weapon carries or upgrades improve the supported package—not for one naked Gold carry.", leave: "Leave when the board lacks Burn frequency, Damage scaling, or first-cycle timing rather than weapon quality." },
      { name: "Tok’s Clocks", why: "Haste and cooldown reduction solve Ignition’s slow baseline, make Trollosaur's nine seconds playable, and help a scaled Rifle reach shot two; Slow buys the time to get there.", reroll: "Reroll when carry output is sufficient but the payoff fires too late.", leave: "Skip when you still lack a meaningful Burn engine or supported carry." },
      { name: "Tinker", why: "Friend-only stock is the most focused Trollosaur route and can also expose Diana-Saur, its cleanest early destroy partner.", reroll: "Reroll only when Trollosaur, Diana-Saur, and other useful Friends give the shop several live hits.", leave: "Do not pay repeated 3g rerolls for one exact Dinosaur or when acceleration is still missing." },
    ],
    timeline: [
      { phase: "Days 1–3", title: "Build events, not final-board clutter", text: "Cheap Burn and Blast Doors stabilize the opener. Track how many distinct Burn applications happen early, but keep saving for Gold access." },
      { phase: "Days 4–6", title: "Test a supported conversion", text: "Buy Rifle only with three pre-shot Burn events; buy Trollosaur only with Damage scaling, acceleration, or a real destroy trigger. Route Hef or Aila only with multiple hits." },
      { phase: "Days 7+", title: "Scale and protect the selected carry", text: "For Rifle, make shot two realistic; for Trollosaur, add Damage, speed, and reliable destruction. If the supported carry still cannot beat sustain or control, exit into Chronobarrier, Flamethrower, or a one-shot." },
    ],
    pivots: ["Plasma Rifle converts Burn-event frequency into direct damage without discarding the engine.", "Trollosaur is a conditional hybrid when Damage scaling and destruction make its proportional Burn and bonus uses relevant.", "Flamethrower inherits Burn and Haste support when the conversions are unavailable or under-supported.", "Boulder, Soul of the District, or Chronobarrier can turn the early economy lead into a faster finish."],
    traps: ["Treating Plasma Rifle as a Burn item and expecting Ignition to grant it Burn.", "Buying the Gold Rifle before three or more Burn events can precede its first shot.", "Following old Trollosaur guides that describe permanent Damage gain on destruction; the live Patch 16.1 card has no permanent-scaling line.", "Counting Trollosaur's Burn magnitude as multiple Rifle stacks—each use creates one Burn application event.", "Running two slow Medium carries without enough acceleration or defense."],
    patchChange: "Patch 16 changed the Core’s Burn from a flat 4 to 4 / 8 / 12. Plasma Rifle and Trollosaur were unchanged in 16.1; Trollosaur's live Diamond hit is 200 and its Burn remains 10% of current Damage.",
    sources: ["official", "expert", "bazaarDb", "plasmaRifle", "trollosaur", "ignitionBuild"],
  },
  {
    id: "primal-core",
    name: "Primal Core",
    ...
    summary:
      "An adjacency puzzle with a strong small-item opening. It makes two neighboring items into Relics, so you can loop premium non-Relics; it also directly scales Dinosaur Weapons such as Trollosaur, though late boards often keep the carry and bench the Core.",
    plan:
      "Exploit Flint Stones and efficient Tools for early wins and economy, then transition to Dooltron, Flamethrower, Press, or a supported Trollosaur. Primal's Damage buff raises both Trollosaur's hit and its 10%-of-Damage Burn.",
    ...
    builds: [
      ...
      { name: "Trollosaur destroy carry", label: "Conditional Dinosaur", confidence: "situational", pitch: "Turn Primal's Dinosaur Weapon scaling into direct Damage, proportional Burn, and destruction-triggered bonus casts.", enter: "Trollosaur appears with Haste or Charge plus Diana-Saur, another early destroy, or opponents exposing useful Small targets.", engine: "Every Primal use raises Trollosaur's Damage and therefore its 10%-of-Damage Burn; every Dinosaur use charges Primal, while successful destruction immediately reuses Trollosaur.", mustHave: ["Trollosaur", "Primal-before-carry timing", "acceleration or reliable destroy"], flex: ["Diana-Saur / Grav Well", "Shiny / Obsidian", "Crit", "defense"], pivot: "Do not force the package from a naked nine-second Trollosaur. Condense into another premium carry when destruction is unreliable or the fight ends before cast two." },
    ],
    buy: ["Flint Stones", "Power Drill", "Soldering Gun / Power Sander", "supported Trollosaur or another premium late carry"],
    hold: ["Diana-Saur or targeted Charge for a Trollosaur line", "Plasma Grenade", "SMG / Uzi", "utility Tools that keep earning a slot"],
    skip: ["unsupported nine-second Trollosaur", "Relics that are only tags", "low-impact adjacent items", "upgrades to filler after the late carry appears"],
    merchants: [
      ...
      { name: "Tinker", why: "Friend-only stock can expose Trollosaur and Diana-Saur together, giving the Dinosaur transition both a carry and its cleanest destroy scaler.", reroll: "Reroll only when several Friends, Dinosaur upgrades, or current-board improvements are live.", leave: "Leave when the route still lacks speed or Damage; one exact Dinosaur is not worth repeated 3g misses." },
    ],
    timeline: [
      ...
      { phase: "Days 4–6", title: "Upgrade or test the handoff", text: "A practical Tool order is Flint, Drill, Grenade, Sander, then Uzi. Test Trollosaur only when Primal can act first and speed or destruction supports it." },
    ...
    pivots: ["Dooltron converts broad tags into late power.", "Trollosaur preserves the Dinosaur Weapon scaling when its Damage, speed, and destroy conditions are already solved.", "Flamethrower or Press can keep the best acceleration while replacing early filler."],
    traps: ["Keeping every Tool because it was good on Day 3.", "Treating Trollosaur's once-per-fight half-Health trigger as guaranteed when the opponent has no Small item.", "Ignoring the two adjacency-granted Relic slots.", "Missing the late transition window while rich."],
    ...
    sources: ["official", "expert", "bazaarDb", "trollosaur"],
  },
  {
    id: "weaponized-core",
...
export const coreDepthById: Record<string, CoreDepth> = {
  ...
  "ignition-core": {
    northStar: "Front-load enough Burn to beat decay, then convert proven support into Plasma Rifle's event scaling or Trollosaur's Damage-and-destruction hybrid without weakening the first cycle.",
    bottleneck: "Burn loses one stack every half-second and deals half damage to Shield. Rifle needs several pre-shot applications; Trollosaur needs Damage, speed, and a credible destruction event. Neither fixes an unsupported board.",
    firstBreakpoint: "For Rifle, forecast three Burn applications before its actual first activation—six seconds at base speed. For Trollosaur, require at least two of three: Core/tag synergy, a reliable use before nine seconds, and existing Damage or Burn scaling.",
    exitRule: "Replace early Burn filler on Day 6. Prefer Rifle when event frequency is solved and Trollosaur when its hybrid conditions are already met; abandon either route if the carry cannot threaten a kill inside two cycles.",
    itemRanks: [
      ...
      { name: "Plasma Rifle", tier: "A", why: "A high-value direct-damage conversion for a supported Burn engine: every separate Burn application adds 100 / 200 fight-long Damage to its six-second base shot." },
      ...
      { name: "Trollosaur", tier: "B", why: "Conditional Burn/Weapon hybrid: its Burn is equal to 10% of current Damage, then Ignition adds flat Burn on top. The nine-second baseline demands acceleration or reliable destruction." },
    ],
    synergies: [
      { name: "Grenade-to-Rocket cycle", grade: "S", items: ["Plasma Grenade", "Rocket Launcher", "Ignition Core", "Haste"], plan: "Sequence Grenade immediately before Rocket so a large Burn packet lands with minimal decay, then use Haste to reach the maintenance cycle.", online: "The first cycle applies enough Burn to keep stacking through Shield and the board survives until the second cycle.", breaks: "If output is sufficient but late, buy speed; if it starts fast but dies, buy survival—not another redundant burner." },
      { name: "Plasma Rifle conversion", grade: "A", items: ["Plasma Rifle", "Ignition Core", "Launch Pad or Multicast Burn", "Metronome or Fiber Optics"], plan: "Forecast at least three Burn applications before Rifle's actual first activation after speed effects—six seconds at base speed. Each event adds 100 / 200 Damage for the fight; speed, Crit, or Multicast then converts the scaled number.", online: "The first shot is roughly 400+ pre-Crit at Gold, shot two is protected and accelerated, and direct damage materially improves the Shield or sustain matchup.", breaks: "Rifle is Weapon / Tech, not Burn: Ignition does not grant it Burn. Skip a naked Rifle, and replace it late if a complete Launcher or pure-Burn cap has clearly higher output." },
      { name: "Trollosaur breakpoint hybrid", grade: "B", items: ["Trollosaur", "Ignition Core", "Damage scaling", "Haste / targeted Charge"], plan: "Place Trollosaur right of Ignition. Damage buffs raise both its direct hit and proportional Burn; Ignition adds flat Burn, and a successful half-Health destruction immediately uses it again.", online: "Trollosaur has real Damage scaling, can act before the fight is decided, and an opposing Small item or external destroy makes the bonus use credible. Shiny adds throughput; Obsidian doubles Damage and therefore the derived Burn; Radiant protects the slow carry.", breaks: "An unaccelerated nine-second copy is not a Plasma engine. Each Trollosaur use contributes exactly one Burn application—and therefore one Rifle stack—regardless of Burn size." },
    ...
    spotlight: {
      item: "Plasma Rifle",
      spec: "GOLD+ · MEDIUM · WEAPON / TECH · 6 SEC",
      title: "Burn events become direct damage",
      summary: "Plasma Rifle is the supported bridge from Ignition's early Burn tempo into a weapon that can pressure Shield and healing. It is an A-tier global opportunity and a situational Ignition conversion only after the event engine is proven—not a blind late-game force.",
      mechanic: "Deal 100 / 200 Damage. Whenever one of your items or skills applies Burn, this gains 100 / 200 Damage for the fight.",
      rule: "Count Burn applications, not Burn ticks or stack size. Plasma Rifle is Weapon / Tech—not Burn—so Ignition's right-side Burn gain does not buff it directly.",
      threshold: "Practical gate: forecast at least 3 distinct Burn events before the Rifle's actual first activation after speed effects—6 seconds at base speed. That is roughly 400 Gold-tier Damage before Crit; 5 events is roughly 600.",
      sequence: [
        { label: "01 · CREATE", text: "Use Gold or Diamond Ignition, Launch Pad, Multicast Soldering, Omega Ray, or a Hasted or Charged Rocket to create pre-shot Burn events." },
        { label: "02 · ORDER", text: "Make those events resolve before the Rifle. With tied cooldowns, place Burn activators earlier and verify the first cycle." },
        { label: "03 · CONVERT", text: "Crit, Shiny, and Obsidian multiply output; Shielded, Restorative, and Toxic scale secondary effects from current Damage." },
        { label: "04 · REPEAT", text: "Use Metronome, Fiber, Haste, Charge, Flying CDR, or protection to make the second scaled shot realistic." },
      ],
      partners: [
        { name: "Launch Pad", why: "The best event engine: every Friend or Flying use applies Burn and adds another Rifle stack. A low-Burn Pad is still useful here." },
        { name: "Soldering Gun", why: "Friend and Tool adjacency can create multiple Burn casts at six seconds. Make it resolve before the Rifle." },
        { name: "Ignition + Omega / Rocket", why: "Gold or Diamond Ignition and Omega can pre-stack Rifle. Base-speed Rocket lands later, so Haste or Charge it for shot one; one giant Burn packet still counts as one application." },
        { name: "Trollosaur", why: "A supplemental Rifle stack per use, not a baseline event engine. Its nine-second cooldown misses Rifle's natural first shot unless Haste, Charge, or an early successful destruction forces a use; Multicast adds uses only after activation." },
        { name: "Metronome / Fiber Optics", why: "Metronome fixes sequencing. Fiber charges Rifle only when Rifle is the literal rightmost item; that targeting is incompatible with placing Thrusters immediately to Rifle's right." },
        { name: "Thrusters + Dino Saddle", why: "Thrusters makes Rifle Flying and a Vehicle; Saddle then adds Multicast. This is the premium Launcher extension." },
        { name: "Party Float / defense", why: "After Launcher or Thrusters makes Rifle Flying, Party Float adds cooldown reduction and Shield; Blast Doors or control can instead buy time for shot two." },
      ],
      upgrades: [
        { name: "Diamond first", why: "Base Damage and every Burn-event gain both double from 100 to 200, making the item upgrade unusually efficient." },
        { name: "Shiny / Obsidian / Deadly", why: "Multicast, double Damage, and Crit directly multiply the number your Burn engine already created." },
        { name: "Shielded / Restorative / Toxic", why: "These conversions scale from the Rifle's current Damage and can turn one huge shot into survival or a second damage axis." },
        { name: "Fiery", why: "Adds a Burn event when Rifle fires, helping later shots; it does not help the pre-first-shot purchase gate." },
        { name: "Event count before Burn amount", why: "Upgrading a Burn source's magnitude improves Burn damage but does not create another Rifle stack." },
      ],
      buyWhen: ["The current board already produces 3+ Burn applications by about five seconds.", "Persistent Haste, meaningful Charge, or Flying support is already available.", "Direct damage materially improves the Shield or healing matchup.", "The 16-gold purchase does not consume the run's survival reserve."],
      avoidWhen: ["The board has one large but infrequent Burn packet.", "The Rifle requires buying several weak burners after the fact.", "No concrete speed, Crit, Multicast, or protection plan exists.", "A complete Ice 9000, Nitrogen Hammer, Terry-Dactyl, or high-scaling Launch Pad finish already has the higher cap."],
      source: { label: "Patch 16.1 tooltip, enchantments, and merchant pools", href: "https://bazaardb.gg/card/szz2k0p3hpz490ffbpyc6s5fwt/Plasma-Rifle" },
    },
    matchup: { favored: "Slow boards that allow Burn events to scale Rifle, plus opponents dependent on a key Small item Trollosaur can disrupt at the half-Health breakpoint.", respect: "Freeze on the carry, damage reduction, immediate-kill lines, and opponents with no destroyable Small item for Trollosaur.", adapt: "Against Shield or healing, convert Burn frequency into Rifle or Drill damage. Keep Trollosaur only when Damage scaling and a real bonus-use path outperform faster Burn or protection." },
  },
  "primal-core": {
    northStar: "Exploit Flint/Tool tempo and Core adjacency to win cheaply, then hand off to a dense carry—or keep Primal when a supported Dinosaur Weapon such as Trollosaur converts every Core use into real scaling.",
    bottleneck: "Early Tools age poorly, the Core's own hit is not a late win condition, and a naked Trollosaur still waits nine seconds for only 50 Damage and 5 Burn at Silver.",
    ...
    itemRanks: [
      ...
      { name: "Trollosaur", tier: "B", why: "Primal directly scales this Dinosaur Weapon, increasing both its hit and 10%-of-Damage Burn. Promote it only with Diana-Saur, acceleration, or another reliable destroy chain." },
    ],
    synergies: [
      ...
      { name: "Diana Trollosaur destroy", grade: "B", items: ["Diana-Saur", "Trollosaur", "Primal Core", "Haste or targeted Charge"], plan: "Diana's first successful Small-item destruction immediately uses Trollosaur and grows Dinosaur Damage; Primal then keeps raising Trollosaur's hit and proportional Burn.", online: "A valid Small target exists, Diana acts early, Primal acts before Trollosaur, and the package has speed or defense for repeat casts.", breaks: "No destroyable Small means no Diana trigger and no forced Trollosaur use. Do not force the package from matching tags alone." },
    ],
  ...
  "weaponized-core": {
    ...
    itemRanks: [
      ...
      { name: "Trollosaur", tier: "B", why: "Place it right of Weaponized: each Core use raises its direct Damage and therefore its 10%-of-Damage Burn. It still needs startup acceleration or destruction support." },
    ],
    synergies: [
      ...
      { name: "Weaponized Trollosaur", grade: "B", items: ["fast left trigger", "Weaponized Core", "Trollosaur", "Haste / destroy support"], plan: "Activate Weaponized before Trollosaur so repeated flat Damage gains enlarge both its hit and proportional Burn; use Haste, Fiber, or destruction to beat the nine-second startup.", online: "Core reliably acts first and Trollosaur gets a meaningful use inside the first cycle rather than waiting for natural cooldown.", breaks: "Matching Weapon and Burn tags are insufficient. Cut it when a faster premium weapon converts the same scaling more consistently." },
    ],
...
export const topDooleyItems: GlobalRankedItem[] = [
  { rank: 1, name: "Fiber Optics", tier: "S", role: "Targeted Charge", summary: "The best universal timing piece: fast left-edge actions become right-edge carry tempo.", bestIn: ["Any Core", "Drill", "Launcher"], why: "It compresses activation order without asking the carry to be fast on its own. Fiber appears in current Drill, Launcher, Shield, and late legendary finishes because a precise Charge is almost never wasted.", condition: "A fast left-edge item and a right-edge Core or carry are both worth accelerating." },
  { rank: 2, name: "Metronome", tier: "S", role: "Haste bridge", summary: "The cleanest bridge from a fast trigger into a Core or primary payoff.", bestIn: ["Any Core", "Weapon", "Status"], why: "Correct placement fixes first-cycle ordering and keeps paying after the opener. It is especially valuable when the Core must act before the carry but the board cannot afford a dedicated speed package.", condition: "Its left neighbor fires early and the right neighbor meaningfully benefits from repeated Haste." },
  { rank: 3, name: "RAMPage Module", tier: "S", role: "Status scaler", summary: "Repeated Slow, Poison, and Freeze become permanent adjacent scaling.", bestIn: ["Power Drill", "Status", "Dooltron"], why: "RAMPage is the defining support behind the current Drill shell and upgrades otherwise modest control pieces into growth engines. Correct adjacency matters more than adding another status tag.", condition: "The board already produces several qualifying status events and both adjacent targets deserve scaling." },
  { rank: 4, name: "Power Drill", tier: "S", role: "Status payoff", summary: "The current payoff that accelerates from nearly every useful status event.", bestIn: ["RAMPage", "Ignition", "Coreless"], why: "Haste, Slow, Freeze, Poison, and Burn all help it cycle, so one coherent control package supplies both setup and damage. Unsupported Drill is slow; supported Drill is a featured Season 16 win condition.", condition: "At least two repeatable status sources are live or RAMPage is already making the route profitable." },
  { rank: 5, name: "Duct Tape", tier: "S", role: "Shield engine", summary: "Cheap defense turns adjacent activations into repeated Shield and Shield triggers.", bestIn: ["Armored", "Harmadillo", "Torch"], why: "Tape is useful early and remains an engine when fast neighbors multiply its procs. It supplies survival, Armored scaling, Harmadillo Charge, and the Shield events that late conversions need.", condition: "Both adjacent slots can activate repeatedly; do not strand it beside passive or disposable filler." },
  { rank: 6, name: "Bunker", tier: "S", role: "Shield Multicast", summary: "Adds Multicast to Shield items while reducing incoming damage.", bestIn: ["Armored", "Harmadillo", "Dooltron"], why: "Few single items multiply a mature Shield board this efficiently. The Large slot is justified when several Shield items gain real extra casts, not when Bunker is being asked to create the plan alone.", condition: "Multiple strong Shield items or one premium enchanted Shield target already justify the Large slot." },
  { rank: 7, name: "Cooling Fan", tier: "S", role: "Freeze opener", summary: "Fast repeatable Freeze plus Burn cleanse for Launcher and Freeze engines.", bestIn: ["Launcher", "Ice 9000", "Nitrogen Hammer"], why: "Its short cooldown provides early Freeze events and useful Tech tempo while cleansing Burn. Current Launcher lines use it as a key startup piece, but self-Freezing the wrong neighbors can delay the board instead.", condition: "A Freeze payoff is live and the adjacent items can tolerate or exploit the self-Freeze." },

  { rank: 8, name: "Scrap Metal", tier: "A", role: "Permanent investment", summary: "Premium permanent cooldown and upgrade value when the run can afford a slow hour.", bestIn: ["Late Core", "Carry routes"], why: "The long-run reward is exceptional, but the cost and delayed payoff are real under Patch 16's tighter economy. It ranks below S because a solvent board uses it brilliantly and a weak board dies holding it.", condition: "The next fight is already covered and the purchase does not consume the reserve for a real tempo shop." },
  { rank: 9, name: "SMG", tier: "A", role: "Fast Ammo opener", summary: "A two-second Ammo weapon that frontloads Core triggers and early weapon tempo.", bestIn: ["Critical", "Weaponized", "Primal"], why: "Rapid uses fix Core-before-carry timing and feed Metronome, Tape, and other activation engines. Ammo makes it an enabler rather than an automatic endboard weapon.", condition: "It improves the first cycle or feeds multiple triggers; replace it when Ammo or slot compression becomes the bottleneck." },
  { rank: 10, name: "Plasma Rifle", tier: "A", role: "Burn-to-damage carry", summary: "Every Burn application adds 100 / 200 Damage to a six-second Gold weapon.", bestIn: ["Ignition", "Launcher", "Burn tempo"], why: "Plasma Rifle converts frequent Burn events into direct damage without discarding the engine that created them. It is a Weapon / Tech rather than a Burn item, so it rewards application count but does not receive Ignition's right-side Burn gain.", condition: "At least three Burn events occur before the first cast and real acceleration, Crit, or protection makes shot two realistic." },
  { rank: 11, name: "Launch Pad", tier: "A", role: "Flying Burn engine", summary: "A Flying toggle plus Burn on every Friend or Flying activation.", bestIn: ["Launcher", "Companion", "Ignition"], why: "Inside its route, Launch Pad turns normal engine actions into both status pressure and Burn scaling. Outside Friend or Flying density, its ceiling is irrelevant and the Property slot is speculative clutter.", condition: "Several real Friend or Flying activations are already forecast in the first combat cycle." },
  { rank: 12, name: "Pulse Rifle", tier: "A", role: "Multicast weapon", summary: "An efficient weapon that gains Multicast beside a useful Friend.", bestIn: ["Weaponized", "Critical", "Companion tempo"], why: "Its compact damage and Friend-enabled Multicast make it one of the cleanest weapon carries to scale. The Friend must still justify its own slot; manufacturing a dead neighbor destroys the efficiency.", condition: "The Rifle is upgraded and one independently useful Friend can activate the Multicast condition." },
  { rank: 13, name: "Party Float", tier: "A", role: "Flying defense", summary: "Flying Shield engine and cooldown support that stabilizes Launcher startup.", bestIn: ["Launcher", "Flying", "Shield"], why: "Party Float turns the same Flying actions that power Launcher into survival and faster cycling. It is premium after an endpoint exists, but does not replace the endpoint.", condition: "At least two meaningful Flying activations are live and the board still needs defense or startup speed." },
  { rank: 14, name: "Dooltron", tier: "A", role: "Status finisher", summary: "A late status and Friend finisher that compresses many small triggers into one threat.", bestIn: ["Companion", "Launcher pivot", "Coreless status"], why: "Haste, Slow, Poison, Freeze, Burn, and Friend uses all support the same large payoff. Once trigger density is high enough, speed or Multicast improves it more than collecting additional tags.", condition: "The board already creates frequent status or Friend events and can accelerate the long base cooldown." },
  { rank: 15, name: "Cool LEDs", tier: "A", role: "Slow utility", summary: "Compact Slow and Core utility that supplies status routes with a clean trigger.", bestIn: ["Any Core", "Drill", "RAMPage"], why: "It disrupts the opponent, feeds status payoffs, and avoids adding another weak weapon. Its job is engine density and timing, not finishing the fight by itself.", condition: "Slow advances a live payoff or its activation improves Core timing; skip it as isolated control." },
  { rank: 16, name: "Lightbulb", tier: "A", role: "Tech + Charge", summary: "Generates Tech and charges adjacent Tech carries such as Welding Torch.", bestIn: ["Welding Torch", "Tech", "Drill"], why: "The small footprint makes Lightbulb a clean accelerator in the current Shield/Burn package. Its target restriction makes board mapping mandatory before purchase.", condition: "A specific adjacent Tech item is already the carry or engine you want to Charge repeatedly." },
  { rank: 17, name: "Chemsnail", tier: "A", role: "Dual-status engine", summary: "One small Friend supplies both Slow and Poison for status packages.", bestIn: ["RAMPage", "Power Drill", "Dooltron"], why: "Two relevant statuses in one compact slot give Chemsnail excellent engine density. It becomes exceptional with repeatable Slow or Friend acceleration, not merely because both keywords appear on the card.", condition: "Slow or Friend triggers already repeat, and Poison contributes to a real payoff rather than decorative damage." },
  { rank: 18, name: "BLK-SP1D3R", tier: "A", role: "Multi-target Slow", summary: "Dense Slow that charges from adjacent Poison or Burn.", bestIn: ["Drill", "Ignition", "Dooltron"], why: "Correct adjacency converts an existing status engine into frequent multi-target disruption and more Drill or RAMPage events. Low tier or unsupported copies are much less impressive.", condition: "An adjacent Poison or Burn source activates repeatedly and the resulting Slow advances the main plan." },
  { rank: 19, name: "Welding Torch", tier: "A", role: "Shield-to-Burn", summary: "Converts a mature Shield engine into Burn inevitability.", bestIn: ["Armored", "Wallace", "Bunker"], why: "Torch gives defensive boards a real clock by turning Shield production into damage. It is a current Season 16 meta route because the support pieces also keep the board alive.", condition: "Repeatable Shield is already online; do not buy Torch first and hope the defensive engine appears later." },
  { rank: 20, name: "Wallace", tier: "A", role: "Fast Shield Friend", summary: "A three-second Friend supplies frequent Shield activations for Torch and Bunker.", bestIn: ["Welding Torch", "Companion", "Shield"], why: "Wallace is one of the cleanest repeatable actions in the current Shield/Burn shell. Patch 16.1 replaced its old per-use growth with flat 10/20/40/80 Shield, so speed increases frequency but no longer compounds Wallace's own scaling.", condition: "The board benefits from repeated Shield or Friend uses; upgrade quality matters more than speculative cooldown stacking." },
  { rank: 21, name: "Harmadillo", tier: "A", role: "Shield damage loop", summary: "A Shield-triggered damage loop that plays like the defensive Power Drill.", bestIn: ["Armored", "Duct Tape", "Coreless"], why: "Every repeatable Shield source helps it cycle, allowing the same compact engine to defend and attack. Its accessible components make it a practical route from early tempo to a real finish.", condition: "Duct Tape or at least two repeatable Shield sources are already producing several events per cycle." },
  { rank: 22, name: "Monitor Lizard", tier: "A", role: "Haste-to-Poison", summary: "Friend-heavy boards turn repeatable Haste into a compact Poison engine.", bestIn: ["Companion", "Drill", "Dooltron"], why: "Each Haste event becomes Poison, so external Charge and speed pieces scale both startup and output. Lower-tier copies can bridge the midgame, but late boards need repeated Haste or a status payoff.", condition: "Multiple Haste events are forecast and Poison advances a live win condition or RAMPage loop." },
  { rank: 23, name: "Ice 9000", tier: "A", role: "Freeze finisher", summary: "A Freeze endpoint with enough control and scaling to finish Launcher loops.", bestIn: ["Launcher", "Freeze", "Drill"], why: "Current Season 16 meta coverage names Ice 9000 as a primary Launcher endpoint. External Freeze grows the payoff while Fiber, Metronome, or Launcher solves its startup.", condition: "Repeatable external Freeze and real acceleration are already present; skip a lone unaccelerated copy." },
  { rank: 24, name: "Nitrogen Hammer", tier: "A", role: "Freeze burst", summary: "Every individual Freeze builds this medium weapon into a late carry.", bestIn: ["Launcher", "Critical", "The Core"], why: "The Hammer rewards quantity of Freeze triggers and brings another Freeze event itself. It is a named current Launcher endpoint, but without a second cast or multiple early Freezes it is merely a slow weapon.", condition: "At least two early Freeze events are forecast and support makes the second activation realistic." },

  { rank: 25, name: "Dino Saddle", tier: "B", role: "Vehicle Multicast", summary: "A Multicast capstone for the exact Vehicle selected by Thrusters.", bestIn: ["Launcher", "Dooltron", "Force Field"], why: "Saddle can double the right carry and create a winning late package, but its Diamond rarity and Vehicle requirement make it an opportunity rather than a plan to force.", condition: "A premium Vehicle target is already live, or Thrusters can convert the exact carry without weakening the board." },
  { rank: 26, name: "Plasma Grenade", tier: "B", role: "Status burst", summary: "A front-loaded Slow and Burn packet for burst, Drill, and status boards.", bestIn: ["Ignition", "Primal", "RAMPage"], why: "One cast supplies dense control and status events, making the first cycle much stronger. Its limited Ammo and self-Burn keep it out of A when the board cannot exploit that opening packet.", condition: "The self-Burn is survivable and the first cast immediately feeds a status payoff or burst lethal." },
  { rank: 27, name: "Memory Card", tier: "B", role: "Core Crit investment", summary: "A sellable Crit investment that lets a Core inherit value built over time.", bestIn: ["Critical", "Core carry"], why: "Repeated safe uses build Value, then selling converts that investment into permanent Core Crit. It needs time and deliberate cash-out timing, so a late or losing-board purchase is too slow.", condition: "Bought early enough to farm safe fights; sell at the required Crit breakpoint rather than on sight." },
  { rank: 28, name: "Thrusters", tier: "B", role: "Vehicle setup", summary: "Makes one item Flying and a Vehicle while improving its startup speed.", bestIn: ["Launcher", "Dino Saddle", "Flying"], why: "Thrusters selects the target that Flying and Vehicle packages will multiply. The tags are valuable only when a real carry is being converted, not as speculative collection.", condition: "A specific target benefits from Flying or Vehicle now, or Saddle is realistically available this run." },
  { rank: 29, name: "Dooley's Scarf", tier: "B", role: "Freeze protection", summary: "Protects adjacent pieces from Freeze and turns your Freeze events into Haste.", bestIn: ["Power Drill", "Launcher", "Freeze"], why: "Scarf can make self-Freeze asymmetrical while accelerating the protected engine. Without frequent Freeze or vulnerable adjacent carries, the slot is mostly insurance.", condition: "Freeze happens repeatedly and both adjacent items are important enough to protect or Haste." },
  { rank: 30, name: "Beta Ray", tier: "B", role: "Repeatable Freeze", summary: "Core and Ray activations repeatedly charge targeted Freeze and control.", bestIn: ["Launcher", "Fast Core", "Ray"], why: "Patch 16's cooldown improvement made Beta Ray a stronger support piece, and dense Core or Ray actions keep it cycling. It still needs a Freeze payoff or a real control breakpoint.", condition: "Core or Ray actions fire often enough to create repeated casts, and Freeze advances the main route." },
  { rank: 31, name: "Robotic Factory", tier: "B", role: "Friend Multicast cap", summary: "A premium Friend capstone when active Friend density already justifies the Large slot.", bestIn: ["Companion", "Dooltron", "Control Friends"], why: "Friend-wide Multicast can multiply a mature engine and its control effects. It is a poor rescue purchase because generic filler does not become an endboard merely by firing twice.", condition: "At least two strong active Friends gain enough from extra casts to repay the Large late-game slot." },
  { rank: 32, name: "Weakpoint Detector", tier: "B", role: "Slow-fed scaler", summary: "Repeatable Slow charges team damage scaling for a wide weapon board.", bestIn: ["Critical", "Weaponized", "Status"], why: "It bridges a dense Slow engine into real weapon growth, especially before the board condenses to one carry. A single Slow or single damage item leaves too much of the text unused.", condition: "Slow repeats before the first cast and at least two meaningful damage items receive the scaling." },
  { rank: 33, name: "Barbed Wire", tier: "B", role: "Shield-status bridge", summary: "Shield events charge a compact weapon that also Slows two targets.", bestIn: ["Armored", "Harmadillo", "RAMPage"], why: "Barbed Wire connects Shield loops to damage and status without adding a large card. It is excellent glue when Tape, Core, or Bunker produces frequent Shield and mediocre as a generic weapon.", condition: "Several Shield events occur each cycle and the Slow also feeds Drill, RAMPage, or control." },
  { rank: 34, name: "Terry-Dactyl", tier: "B", role: "Flying finisher", summary: "A Launcher endpoint that converts repeated Flying changes into heavy damage.", bestIn: ["Launcher", "Flying", "Primal"], why: "Flying and Dinosaur actions Charge its long toggle, and stopping Flying releases the payoff. Current meta names it among legitimate Launcher finishes, but the Gold version and thin trigger boards are not enough late.", condition: "Several real Flying or Dinosaur actions plus acceleration can produce repeatable stops; Diamond quality is preferred." },

  { rank: 35, name: "Crypto", tier: "C", role: "Economy hedge", summary: "A volatile economy hold with no combat text; its tier measures board power.", bestIn: ["Any route while ahead"], why: "Hourly random Value can turn spare gold and stash space into profit, but Crypto contributes nothing to the next fight. Treat it as an economy decision, not evidence that a combat package is forming.", condition: "The cost does not weaken the next fight or block a targeted shop; sell on a favorable roll or when tempo needs cash." },
  { rank: 36, name: "Dooltron Mainframe", tier: "C", role: "Dual accelerator", summary: "Friend uses charge a Core and Haste a Large item while enabling Dooltron as a Core.", bestIn: ["Dooltron", "Companion", "Launcher hybrid"], why: "The ceiling is high because one Friend action can support two premium targets. The requirement is equally narrow: without both Friend density and a worthwhile Core or Large payoff, Mainframe is passive board tax.", condition: "An actual Dooltron or premium Large target and at least two active Friends are already part of the board." },
  { rank: 37, name: "Force Field", tier: "C", role: "Shield cash-out", summary: "A large Shield-to-damage endpoint with excellent ceiling and severe setup cost.", bestIn: ["Armored", "Harmadillo", "Coreless Shield"], why: "It gives defense-heavy boards a direct kill condition, but only the Shield present at cast time makes the damage relevant. A slow, unaccelerated copy bought from zero Shield is not a plan.", condition: "The first-cast Shield forecast is already lethal-scale and Charge, Haste, or Crit supports a second threat." },
  { rank: 38, name: "Soldering Gun", tier: "C", role: "Multicast Burn bridge", summary: "A small Burn Tool gains Multicast from adjacent Friend and Tool tags.", bestIn: ["Ignition", "Primal", "Drill"], why: "Correct neighbors turn a modest early Burn source into several activations and status events. Preserving weak cards solely to satisfy both adjacency conditions usually costs more than the extra casts provide.", condition: "At least one adjacency is free early; demand both conditions or another payoff before keeping it late." },
  { rank: 39, name: "Combat Core", tier: "C", role: "Two-sided scaler", summary: "A Large self-scaler gains from both sides and supplies damage plus Shield.", bestIn: ["Critical", "Weaponized", "Double Core"], why: "Frequent left and right activations can grow it into a self-contained fallback carry. Its Large Gold starting point, six-second cycle, and positioning demands make unsupported copies a downgrade.", condition: "Useful activations exist on both sides, the item is upgraded, and Fiber or Haste solves startup." },
  { rank: 40, name: "Cybersecurity", tier: "C", role: "Tag-density weapon", summary: "A Gold Friend weapon scales from Weapon and Tech density and triples as the lone Friend.", bestIn: ["Critical", "Weaponized", "Tech"], why: "Patch 16.1 gives it substantial damage for each other Weapon and Tech, with a powerful solo-Friend multiplier. The eight-to-nine-second base cycle still demands a dense tag count and acceleration.", condition: "Several counted Weapon or Tech tags are live, or the solo-Friend triple is deliberate, and speed support exists." },
];
...
export const merchantDirectory = [
  ...
  { name: "Mittel", stock: "Medium items", reroll: "2g · 1 reroll", bestFor: "Core engines, Plasma Rifle or Trollosaur conversions, midgame board density", rule: "The default best size shop when two or more Medium upgrades are live." },
  { name: "Aila", stock: "Weapons", reroll: "2g · 1 reroll", bestFor: "Critical, Weaponized, Plasma Rifle, and supported Trollosaur carries", rule: "Visit for weapon quality, then leave once output is solved." },
  ...
  { name: "Tinker", stock: "Friends", reroll: "3g", bestFor: "Companion, Launcher, and Trollosaur / Diana-Saur packages", rule: "Reroll while several Friends improve the engine—not for one exact Dinosaur." },
  ...
  { name: "Hef", stock: "Burn items", reroll: "3g · 1 reroll", bestFor: "Ignition, Plasma Rifle, Trollosaur, and focused Burn finishers", rule: "The narrow Burn shop; Trollosaur is a true Burn hit, but leave when Damage, cooldown, or survival is the bottleneck." },
  ...
  { name: "The Tester", stock: "Tech from several Heroes", reroll: "3g · 1 reroll", bestFor: "Plasma Rifle, cross-Core Tech, and unusual support", rule: "High ceiling, broad pool. Enter with several valid Tech outcomes." },
